  ...
  blockManager: {
    ...
    blocks: [
      ...
      },
      {
        id: 'sub_title',
        label: 'Sub Title',
        content: '<h2 data-gjs-type="title">Insert your sub title here</h2>',
      },
      {
      ...
      requestAnimationFrame(() => {
        ...
        this.editor.Commands.add('save', {
          run: editor => fetch(window.location.protocol  + "//" + window.location.host + '/content/post/' + self.post.id, {
            ...
            body: JSON.stringify({
              html: juice(editor.getHtml() +'<style>' + editor.getCss().replace(/;/g, ' !important;') + '</style>'),
              css: editor.getCss()
            })
          }).then(console.log)
        });
...
<style>
.panel__devices, .panel__switcher, .panel__actions {
  position: initial;
}
.panel__top {
  padding: 0;
